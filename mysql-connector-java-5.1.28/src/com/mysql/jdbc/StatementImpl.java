...
public class StatementImpl implements Statement {
	...
	protected void implicitlyCloseAllOpenResults() throws SQLException {
		...
		try {
			if (!(connection.getHoldResultsOpenOverStatementClose() ||
		...
		try {
			synchronized (checkClosed().getConnectionMutex()) {
				if (isCloseOnCompletion()
					...
					&& (this.generatedKeysResults == null ||
						!this.generatedKeysResults.reallyResult() ||
						this.generatedKeysResults.isClosed())) {
	...
	protected ResultSetInternalMethods getGeneratedKeysInternal(int numKeys)
			throws SQLException {
		synchronized (checkClosed().getConnectionMutex()) {
			Field[] fields = new Field[1];
			fields[0] = new Field("", "GENERATED_KEY", Types.BIGINT, 17); //$NON-NLS-1$ //$NON-NLS-2$
